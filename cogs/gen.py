...
8. winter [text]: Talk with the bot!
    ...
    async def image(self, ctx, *,args:str= None):
        '''search image'''
        if args == None:
            await ctx.send("`Error: No Key words Provided!`")
            return
        FLICKR_PUBLIC = '{}'.format(os.environ.get("public"))
        FLICKR_SECRET = '{}'.format(os.environ.get("secretsss"))
        flickr = FlickrAPI(FLICKR_PUBLIC, FLICKR_SECRET, format='parsed-json')
        term = args.lower()
        search = flickr.photos.search(text=term, per_page=5, extras='url_o')
        en = randint(0,4)
        try:
            photos = search['photos']['photo'][en]['url_o']
            if not photos:
                await ctx.send("`-None-`")
            else:
                em = discord.Embed()
                em.set_image(url = "{}".format(photos))
                em.colour = discord.Colour.dark_blue()
                await ctx.send(embed = em)
        except Exception as e:
