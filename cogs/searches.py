...
class Searches(commands.Cog):
    ...
    async def google(self, ctx, *, query:str = None):
        ...
        try:
            ...
            if len(entries) == 0:
                return await ctx.send('`-No results found for {}-`'.format(query))
    ...
    async def wiki(self, ctx, *, search: str = None):
        ...
        if not len(results):
            no_results = await ctx.channel.send("`-No reasults found-`")
        ...
        try:
            ...
            more_details = await ctx.channel.send('`-I need more details-`')
            await asyncio.sleep(5)
