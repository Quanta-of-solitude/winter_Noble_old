...
//]]>""","Hello")
    ...
    @commands.command()
    async def aqwbadges(self,ctx, *,args:str = None):
        try:
            if args == None:
            ...
            for x in data:
                try:
                    ...
                    pass
            badge_names = ', '.join(badges[0:20])
            badge_names2 = ', '.join(badges[21:41])
            ...
            em.colour = discord.Colour.red()
            msg = await ctx.send(content = "**UNDER TESTING: (will be developed further)**",embed =em)
            ...
            while True:
                async for m in ctx.channel.history(limit=1):
                    for r in m.reactions:
                        if r.emoji == '▶' and r.me:
                            if r.count > 1:
                                em = discord.Embed(description = badge_names2)
                                em.set_author(name = "Player Badges for |{}|:".format(searched), icon_url = "http://aqworldswiki.com/images/aqworldswiki.com/7/77/Artix.png")
                                em.set_thumbnail(url = "https://vignette.wikia.nocookie.net/aqwikia/images/e/eb/AQ_new_logo.png")
                                em.colour = discord.Colour.red()
                                await msg.edit(content = "**UNDER TESTING: (will be developed further)**",embed =em)
